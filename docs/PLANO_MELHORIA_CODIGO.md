...
> **Status:** ✅ Já feitos — mapeamento por cabeçalho (A1), ID por transação (A2), validação server-side (B1), CRUD completo (B2), constantes/enums parciais, Git/GitHub (E2). ⏳ Pendentes — cache (B3), split em módulos (B4), testes formais (B7), PWA real/migração (D1). Ver [CHANGELOG.md](CHANGELOG.md).
